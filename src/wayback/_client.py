        ...
        while next_query:
            ...
            for line in lines:
                ...
                try:
                    ...
                    urlkey, raw_timestamp, original, mimetype, raw_status, digest, raw_length, *_ = text.split(' ')
                ...
                original = REDUNDANT_HTTPS_PORT.sub(r'\1\2', REDUNDANT_HTTP_PORT.sub(r'\1\2', original))
        ...
        while True:
            ...
            if is_memento:
                ...
                if not playable:
                    ...
                    if (
                        ('AdministrativeAccessControlException' in message)
                        or ('URL has been excluded' in response.text)
                    ):  # fmt: skip
                        raise BlockedSiteError(f'{url} is blocked from access')
                    elif (
                        ('RobotAccessControlException' in message)
                        or ('robots.txt' in response.text)
                    ):  # fmt: skip
